...
if __name__ == "__main__":
   ...
   #Data to metrics
    ...
    for i in range(len(dataset)): #len(dataset) 
        ...
        option_prompt = no_help.options_prompt(description, action)
    ...
    agg_metrics_df.to_csv(f"{exp_res_dir}/noplans_nohelp_agg_metrics_{i}.csv") #поправить записььь

    metrics = pd.DataFrame(metrics_batch)
    metrics.to_csv(f"{exp_res_dir}/noplans_nohelp_metrics_{i}.csv")
    
    amb_dif = ambiguity_differentiation(metrics)
    print(amb_dif)
    with open (f"{exp_res_dir}/noplans_nohelp_ambdif_{i}.txt", 'a') as file:
