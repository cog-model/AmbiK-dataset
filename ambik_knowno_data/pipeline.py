...
from ambik_knowno_data.prompts import examples_generation, answer_generation
...
class KnowNoPipe():
    ...
    def generate_answer(self, prompt, description, task): #choosing CP set for single example
        llm = LLM(self.config.answering['model'],
                  self.config.answering['generation_kwargs'])
        
        prompt = self.answer_prompt(prompt, description, task)
    ...
    def run_batch(self, option_prompts, tasks_for_ans): #run, but for batch
        ...
        for i in range(len(options)):
            answer_prompts.append(self.answer_prompt(options[i], tasks_for_ans[i]['description'], tasks_for_ans[i]['task']))
...
if __name__ == "__main__":
    ...
    configs = parse_config("./configs/ambik_knowno_data.yaml" , use_args=True)
    ...
    dataset = pd.read_csv("./ambik_knowno_data/knowno_data_test_200.csv") #ambik_test_400.csv #ambik_test_for_testing.csv test_400
   
   #Data to metrics
    amb_type = dataset['ambiguity_type'].values
    intents = dataset['user_intent'].values
    amb_shortlist = ['0'] * len(dataset)
    ...
    for i in range(len(dataset)): #len(dataset) 
        ...
        tasks_for_ans.append({'description':description, 'task':task})
        option_prompt = knowno.options_prompt(description, task)
    ...
    agg_metrics_df.to_csv(f"{exp_res_dir}/ambik_knowno_data_agg_metrics_{i}.csv") #поправить записььь

    metrics = pd.DataFrame(metrics_batch)
    metrics.to_csv(f"{exp_res_dir}/ambik_knowno_data_metrics_{i}.csv")