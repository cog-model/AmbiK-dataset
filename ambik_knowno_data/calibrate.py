...
from ambik_knowno_data.pipeline import KnowNoConfig, KnowNoPipe

def get_logits(knowno, description, task): #def get_logits(knowno, prompt)
     options = knowno.predict_examples(description, task) 
     gc.collect()
     choose = knowno.generate_answer(options, description, task)
     gc.collect()
...
def calibration():
    ...
    #Calibration set
    dataset = pd.read_csv("./ambik_knowno_data/knowno_data_calib_100.csv") #calib_100

    calibration_data = []
    
    calib_data = pd.DataFrame(columns=['id', 'task', 'user_intent'])
    for i in range(len(dataset)): #len(dataset)
        description = dataset.loc[i, 'environment_full']
        task = dataset.loc[i, 'task']
         
        options, answers_logits = get_logits(knowno, description, task)
        options_to_filter = {}
        ...
        variants = [dataset.loc[i, 'user_intent']]
        ...
        row = {'id':dataset.loc[i, 'id'], 'all variants':", ".join(options.values()),
    ...
    model = model.split('/')[-1]
    calib_data.to_csv('calib_data/knowno_data_' + model +'.csv')
