...
from noplans_binary.prompts import examples_generation, uncertainty_detection
...
class BinaryPipe():
    ...
    def options_prompt(self, description, action):
    ...
    def unc_prompt(self, option, description, action):
    ...
    def run_batch(self, option_prompts, tasks_for_ans): #run, but for batch
        ...
        for i in range(len(options)):
            unc_prompts.append(self.unc_prompt(options[i][0], tasks_for_ans[i]['description'], tasks_for_ans[i]['action']))
...
if __name__ == "__main__":
   ...
   #Data to metrics
    ...
    for i in range(len(dataset)): #len(dataset) 
        ...
        tasks_for_ans.append({'description':description, 'action':action})
        option_prompt = binary.options_prompt(description, action)
    ...
    agg_metrics_df.to_csv(f"{exp_res_dir}/nohelp_binary_agg_metrics_{i}.csv") #поправить записььь

    metrics = pd.DataFrame(metrics_batch)
    metrics.to_csv(f"{exp_res_dir}/nohelp_binary_metrics_{i}.csv")


    metrics, amb_dif = ambiguity_differentiation(metrics)
    print(amb_dif)
    with open (f"{exp_res_dir}/nohelp_binary_ambdif_{i}.txt", 'a') as file:
