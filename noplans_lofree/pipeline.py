...
class LoFreePipe():
    def __init__(self, config=None):
        self.config = config
        self.prompting_number = 5 #за раз енерируется 4 варианта, prompting_number = количество желаемых вариантов (20)/4
        self.lambda1 = 0.1
        self.lambda2 = 0.1
        self.cp = 0.991592680995454 #поменять на свое значение СР, посчитанное запуском calibration.py
...
if __name__ == "__main__":
    ...
    for i in range(len(test_set)): #len(test_set)
        ...
        if i%25 == 0:
            ...
            agg_metrics_df.to_csv(f"{exp_res_dir}/noplans_lofree_agg_metrics_{i}.csv")        
            metrics = pd.DataFrame(metrics_batch)
            metrics.to_csv(f"{exp_res_dir}/noplans_lofree_metrics_{i}.csv")
    ...
    print(amb_dif)
    with open (f"{exp_res_dir}/noplans_lofree_ambdif_{i}.txt", 'a') as file:
