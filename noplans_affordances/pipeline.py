...
from noplans_affordances.prompts import examples_generation, affordance_generation, answer_generation
from noplans_affordances.environment_objects import environment_objects

CP = 2.7181292346055885
...
class AffordancesPipe():
    ...
    def generate_affordances(self, sample): #calculate affordance scores
        ...
        for j in range(len(li)):
            ...
            affordance_prompt = self.affordance_prompt.replace('<ACT>', action)
    ...
    def generate_answer(self, sample):
        ...
        #prompt = prompt.replace("You", "Options")
        prompt = answer_generation.replace('<DESCRIPTION>', sample['description'])
...
if __name__ == "__main__":
    ...
    for i in range(len(dataset)): #len(test_set)
        ...
        if isinstance(amb_shortlist[i], str):
            ...
            sample_keywords = -1
        metrics = _calculate_metrics(llm_answers, sample['affordance_scores'], amb_type[i], intents[i].split(', '), sample_keywords)
        for key in metrics:
            metrics_batch[key].append(metrics[key])
        if i%10 == 0:
           agg_metrics = aggreate(metrics_batch)
           agg_metrics_df = pd.DataFrame(agg_metrics)
           agg_metrics_df.to_csv(f"{exp_res_dir}/noplans_affordances_agg_metrics_{i}.csv")        
           metrics = pd.DataFrame(metrics_batch)
           metrics.to_csv(f"{exp_res_dir}/noplans_affordances_metrics_{i}.csv")


    df, amb_dif = ambiguity_differentiation(metrics)
    print(amb_dif)
    with open (f"{exp_res_dir}/noplans_lap_ambdif_{i}.txt", 'a') as file:
